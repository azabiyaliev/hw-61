  ...
  useEffect(() => {
    const getCountryByAlphaCode = async () => {
      ...
      const bordersCountries = responseBorders.map(async (border) => {
        const resBorders: {data:ICountryMutation} = await axios(URL_TO_GET_ONE_COUNTRY + border);
    ...
    <>
      <Container>
        <Row className="mt-4">
          <Col sm={8}>
            <h1>{country.name}</h1>
            <p className="mt-5"><b>Capital:</b> {country.capital ? country.capital : 'Not have capital'}</p>
